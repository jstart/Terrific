...
- (id) init
{
    if (self = [super init])
    {
        self.itemSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 200);
    }
    return self;
}
...
- (UICollectionViewLayoutAttributes *) initialLayoutAttributesForAppearingItemAtIndexPath:(NSIndexPath *)itemIndexPath
{
    ...
    frame.origin.y = ((self.itemSize.height + self.minimumLineSpacing) * attr.indexPath.item) + self.itemSize.height / 3; // 40 is 2*cellSpacing
